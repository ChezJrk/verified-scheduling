...
Hint Extern 6 (_ < _)%Z => lia || zomega : crunch.
Hint Extern 6 (_ < _) => lia || zomega : crunch.
Hint Extern 6 (_ <= _) => lia || zomega : crunch.
Hint Extern 6 (_ = _) => lia || zomega : crunch.
...
Proof.
  ...
  apply sumr_bound_indic with t0;
    autorewrite with crunch;
    auto with crunch.
  destruct N; simpl in *; try lia; try zomega.
...
Proof.
  ...
  - unfold iverson. simpl.
    ...
    apply mul_0_idemp;rewrite H8;
    rewrite Z2Nat.id; auto with crunch;
      destruct N; lia; zomega; simpl in H0;
...
Proof.
  ...
  destruct (i <? Z.of_nat (n + m))%Z eqn:e; unbool;
      destruct (0 <=? i)%Z eqn:ee; unbool; try lia.
  - rewrite get_gen_some by lia.
    reflexivity.
  - rewrite get_gen_neg_null; try lia.
