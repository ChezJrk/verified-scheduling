...
Proof.
  ...
  match goal with |- ?prog = _ => erewrite <- transpose_transpose_id with
      (V := prog) end.
  reflexivity. consistent_shape. rewrite Nat2Z.inj_sub. simpl. omega.
  omega. assert (0 < m-1). omega. eassumption.
  rewrite Z2Nat.inj_sub.
  rewrite Z2Nat.inj_sub.
  repeat rewrite Nat2Z.id.
  zify. omega. omega. omega.
  rewrite Z2Nat.inj_sub.
  rewrite Z2Nat.inj_sub.
  repeat rewrite Nat2Z.id.
  replace (m - (m- Z.to_nat 1)) with 1.
  rewrite Nat.sub_add. reflexivity. omega.
  zify. omega. omega. omega. zomega. rewrite Z2Nat.inj_sub.
  rewrite Nat2Z.id. zomega. omega.
  auto with crunch. omega. all: protect.
