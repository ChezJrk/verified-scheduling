...
Module Type S.
  ...
  Parameter transform : forall A B, fmap A B -> (A -> A) -> fmap A B.
...
Module M : S.
  ...
  Theorem lookup_transform : forall A B (m : fmap A B) (f : A -> A) a b,
      a = f b ->
      lookup (transform m f) b = lookup m a.
  Proof.
  ...
  Theorem dom_remove : forall A B (m : fmap A B) k,
    dom (remove m k) = dom m \setminus constant (k::nil).
  Proof.
    unfold dom, remove; simpl; intros.
    sets ltac:(simpl in *; try match goal with
