...
Lemma flatten_trunc_flatten_truncr {X} `{TensorElem X} :
  forall (l : list (list X)) k n m (s: @shape X _),
    consistent l (n,(m,s)) ->
    flatten_trunc k l = trunc_r k (Common.flatten l).
Proof.
  unfold flatten_trunc. intros. unfold trunc_r.
  erewrite consistent_length.
  2: eauto.
  erewrite consistent_length.
  2: eapply consistent_get; eauto.
  eapply gen_eq_bound. intros. unfold Common.flatten.
  erewrite consistent_length.
  2: eauto.
  erewrite consistent_length.
  2: eapply consistent_get; eauto.
  assert (i < Z.of_nat n * Z.of_nat m \/
            Z.of_nat n * Z.of_nat m <= i)%Z
    as Hcase by lia.
  inversion Hcase as [ Hcase1 | Hcase2 ]; clear Hcase.
  - rewrite get_gen_some by lia. reflexivity.
  - destruct m; destruct n.
    + simpl in *. unfold sum,sumr. simpl. unfold gen, genr. reflexivity.
    + simpl in *. unfold sum,sumr. simpl. unfold gen, genr.
      rewrite Z.sub_diag. simpl.
      unfold get. rewrite sum_helper_const. reflexivity.
    + simpl in *. unfold sum,sumr. simpl. unfold gen, genr.
      rewrite Z.sub_diag. simpl.
      unfold get. reflexivity.
    + rewrite get_gen_null. 2: lia.
      etransitivity.
      eapply sum_eq_bound. intros.
      eapply sum_eq_bound. intros.
      assert ((i =? i0 * Z.of_nat (S m) + i1)%Z = false).
      eapply Z.eqb_neq.
      unfold not. intros. subst.
      eapply (mul_add_lt i0 i1 (Z.of_nat (S n)) (Z.of_nat (S m))) in H3; auto.
      lia.
      rewrite H7.
      reflexivity.
      2: lia. simpl.
      ...
      rewrite sum_iverson_lift.
      eapply sum_eq_bound. intros.
      rewrite iverson_mul_false by auto.
      rewrite iverson_mul_false by auto.
      eapply mul_0_absorb.
      consistent_shape.
      consistent_shape.
...
Theorem flatten_truncr_tile_id {X} `{TensorElem X} :
  forall (V : list X) K n s,
    0 < K ->
    consistent V (n,s)->
    trunc_r n (flatten (tile V K)) = V.
Proof.
  intros. erewrite <- flatten_trunc_tile_id; eauto.
  erewrite flatten_trunc_flatten_truncr.
  2: { apply consistent_tile; eauto. }
  reflexivity.
Qed.
...
Theorem flatten_truncr_tile_id' {X} `{TensorElem X} :
  forall (V : list X) K n s,
    0 < K ->
    consistent V (n,s)->
    truncr ((n //n K)* K - n) (flatten (tile V K)) = V.
Proof.
  intros. erewrite <- flatten_trunc_tile_id; eauto.
  erewrite flatten_trunc_flatten_truncr.
  2: { apply consistent_tile; eauto. }
  rewrite trunc_r_truncr.
  2: { unfold flatten.
       rewrite gen_length.
       unfold tile.
       repeat rewrite gen_length.
       destruct V.
       - simpl in *.
         rewrite zero_div by lia. simpl. inversion H1.
       - pose proof H1. eapply consistent_length in H2.
         simpl in H2. destruct n. lia. inversion H2.
         rewrite Z2Nat.id.
         2: { eapply ceil_div_nonneg; simpl; lia. }
         unfold gen, genr.
         rewrite Z.sub_0_r.
         rewrite znat_id_distr.
         repeat rewrite Nat2Z.id.
         pose proof (ndiv_pos (length (x ::V)) K).
         simpl in *. destruct (S (length V) //n K) eqn:e. lia.
         simpl. unfold get. simpl. rewrite Z.sub_0_r. rewrite Nat2Z.id.
  ...
  unfold flatten at 2. rewrite gen_length.
  rewrite Z2Nat.inj_mul by lia.
  repeat rewrite Nat2Z.id.
  erewrite length_of_tile; eauto.
  rewrite znat_id_distr. repeat rewrite Nat2Z.id.
  unfold tile at 2.
  unfold gen, genr.
  rewrite Z.sub_0_r. rewrite znat_id_distr.
  repeat rewrite Nat2Z.id.
  destruct (length V //n K) eqn:ee.
  pose proof (ndiv_pos (length V) K).
  rewrite ee in *.
  inversion H1. subst. simpl in *. lia.
  unfold get. simpl. rewrite Z.sub_0_r. rewrite gen_helper_length.
  rewrite Nat2Z.id. 
  reflexivity.
Qed.
...
Proof.
  ...
  intros. eapply consistent_get. eapply consistent_get.
  eapply H1. lia.
  intros. eapply consistent_gen. 3: reflexivity.
  2: { intros. eapply consistent_get. eapply consistent_get.
       eapply H1. lia. }
...
Proof.
  ...
  erewrite length_add_length. eauto. eauto. eauto. simpl in *. inversion H11.
  lia.
Qed.
